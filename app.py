#!/usr/bin/env python3
"""
UltraLearn IA – Edição Suprema Completa (com Sobrevivência, Molduras, Continuação, etc.)
"""
...
def inject_css(theme="dark", font_size=16, daltonic=None):
    ...
    html, body, [class*="css"] {{ font-family: 'Outfit', 'Plus Jakarta Sans', sans-serif; background-color: {t["bg_main"]}; color: {t["text_color"]}; font-size: {font_size}px; }}
    ...
    div.stButton>button {{ border-radius: 14px; font-weight: 600; padding: 0.7rem 2.5rem; border: none; background: {t["button_bg"]}; color: white; letter-spacing: 0.5px; text-transform: uppercase; font-size: 0.9rem; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 4px 15px rgba(0,0,0,0.3); position: relative; overflow: hidden; }}
    div.stButton>button:hover {{ transform: translateY(-2px); box-shadow: 0 8px 25px rgba(59,130,246,0.4); }}
    div.stButton>button:active {{ transform: translateY(0); }}
    .feedback-correct {{ background: rgba(34,197,94,0.15); border-left: 4px solid #22c55e; padding: 1rem; border-radius: 12px; margin: 1rem 0; color: #bbf7d0; backdrop-filter: blur(5px); }}
    .feedback-incorrect {{ background: rgba(239,68,68,0.15); border-left: 4px solid #ef4444; padding: 1rem; border-radius: 12px; margin: 1rem 0; color: #fecaca; backdrop-filter: blur(5px); }}
    .stProgress > div > div {{ background: linear-gradient(90deg, {t["accent"]}, {t["text_color"]}) !important; border-radius: 10px; transition: width 0.5s ease; }}
    textarea, input {{ background: rgba(255,255,255,0.05) !important; border: 1px solid rgba(255,255,255,0.15) !important; border-radius: 14px !important; color: {t["text_color"]} !important; padding: 0.8rem 1rem !important; backdrop-filter: blur(5px); }}
    .stTabs [data-baseweb="tab-list"] {{ gap: 0.5rem; background: transparent; border-bottom: 2px solid rgba(255,255,255,0.1); }}
    .stTabs [data-baseweb="tab"] {{ color: {t["text_color"]}; font-weight: 500; border-radius: 12px 12px 0 0; padding: 0.6rem 1.2rem; transition: all 0.2s; }}
...
def tela_login():
    ...
    with col2:
        st.markdown("""<div style="margin-top: 2rem;"><h1 style="font-size: 3.5rem; font-weight: 700; background: linear-gradient(135deg, #60a5fa, #a78bfa); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">UltraLearn IA</h1><p style="font-size: 1.3rem; opacity: 0.8;">A plataforma definitiva de aprendizado</p></div>""", unsafe_allow_html=True)
...
def gen_continuacao(topic, texto_anterior):
    prompt = f"Baseado na explicação anterior sobre '{topic}':\n{texto_anterior}\n\nAgora, forneça mais detalhes, curiosidades e aprofundamentos sobre o mesmo tópico, mantendo a linguagem clara e envolvente."
    resp = client.chat.completions.create(model="llama-3.3-70b-versatile",
        messages=[{"role":"user","content": prompt}], temperature=0.8, max_tokens=1500)
    return resp.choices[0].message.content.strip()
...
def show_quiz(questions, mode="normal"):
    ...
    if mode == "sobrevivência":
        ...
        vidas = st.session_state.lives
        st.sidebar.metric("❤️ Vidas", vidas)
    if mode == "relâmpago":
...
def main_app():
        ...
        if st.session_state.explanation:
            ...
            with col_btn2:
                if st.button("🧠 Quero saber mais"):
                    st.session_state.explanation += "\n\n" + gen_continuacao(st.session_state.topic, st.session_state.explanation)
                    atualizar_progresso_missao("usar_continuacao")
                    st.rerun()
            with st.expander("💬 Comentários"):
                comentarios = conn.execute("SELECT user_id, content, created_at FROM comments WHERE topic=? ORDER BY created_at DESC", (st.session_state.topic,)).fetchall()
                for c in comentarios:
                    st.write(f"**{c[0]}** ({c[2]}): {c[1]}")
                novo_comment = st.text_area("Adicione um comentário")
                if st.button("Enviar comentário") and novo_comment:
                    conn.execute("INSERT INTO comments (user_id, topic, content) VALUES (?,?,?)", (USER_ID, st.session_state.topic, novo_comment))
                    conn.commit()
                    atualizar_progresso_missao("comentar")
                    st.rerun()
            ...
            if st.button("Criar Quiz"):
                ...
                st.session_state.quiz_index = 0; st.session_state.quiz_score = 0; st.session_state.quiz_active = True
    ...
    # Demais abas mantidas como anteriormente (Mapa Mental, Debate, História, Música, Redação, Professor, Progresso, Diário)
    # ... (código não alterado, apenas com as correções já existentes)
